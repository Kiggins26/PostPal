...
from editor_models import MLP, normalized_pt, get_transform_to_params
...
def edit_image(dir_path, image_paths):
    # flag/folder/temp
    ...
    runif = np.random.uniform
    # clean tmp
    cur_folder = os.path.dirname(dir_path)
    ...
    for image_pth in image_paths:
        ...
        model_evalaesthetic.load_state_dict(
            torch.load("model/sac+logos+ava1-l14-linearMSE.pth", map_location=device)
        )
        ...
        # evaluate base score
        base_score = measure_aesthetic(
            base_pil_image_c, model_clip, model_evalaesthetic
        )
        ...
        for it in range(it, n_iter):
            ...
            if it < max_it_try_random:
                ...
                if runif(0, 1) < p_non_optimal and not do_optimal:
                    ...
                    # filter good
                    worst_than_best = (
                        node_to_score[prev_node] < best_score - thres_bestscorelow
                    )
                    worst_than_base = (
                        node_to_score[prev_node] < base_score - thres_basescorelow
                    )
                    if worst_than_best or worst_than_base:
                        continue
                # use best node
                else:
                    prev_node = best_node
            ...
            if len(node_to_score) == 0 or score > best_score:
                ...
                if time.time() - last_save_time > 5 or it < 100:
                    apply_node(base_pil_image, best_node).save(
                        join(tmp_folder, f"{id_run}_step_{it}.jpg")
                    )
